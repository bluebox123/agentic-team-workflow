...
const registry_1 = require("../src/brain/registry");
const validator_1 = require("../src/brain/validator");
...
async function runVerification() {
    ...
    // 1. Verify Registry
    console.log(`Registry loaded with ${registry_1.AGENT_REGISTRY.length} agents.`);
    if (registry_1.AGENT_REGISTRY.length === 0)
    ...
    const v1 = (0, validator_1.validateWorkflow)(validDag);
    ...
    const v2 = (0, validator_1.validateWorkflow)(cycleDag);
    console.log("Validation Test 2 (Cycle):", !v2.valid ? "PASS" : "FAIL");
    if (v2.valid)
        throw new Error("Cycle DAG passed validation");
    ...
    const validation = (0, validator_1.validateWorkflow)({
    ...
    if (!validation.valid)
        throw new Error("Parsed workflow invalid");
